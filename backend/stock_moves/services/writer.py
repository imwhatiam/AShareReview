...
def write_stock_move_analysis(
    *,
    ...
    """Replace the result for one source version and mark its run successful."""
        ...
        with transaction.atomic(using='stock_moves'):
            ...
            StockMoveItem.objects.using('stock_moves').bulk_create([
                StockMoveItem(
                    ...
                    parent_industries=list(item.parent_industries),
...
def record_failed_run(*, business_date, error: Exception, source_daily_price_version: str = '') -> None:
