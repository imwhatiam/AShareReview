...
class StockMoveModelTests(TestCase):
    ...
    def test_result_keeps_business_date_and_both_source_versions(self):
        ...
        self.assertEqual(result.source_industry_version, 'industries-20260908-v1')
        self.assertEqual(result.bse_rise_count, 1)
        self.assertEqual(result.bse_fall_count, 2)
        self.assertEqual(result.distinct_stock_count, 10)

    def test_item_keeps_one_of_six_groups_rank_and_parent_industry_snapshot(self):
        result = StockMoveResult.objects.create(
            business_date=date(2026, 9, 8),
            source_daily_price_version='daily-prices-20260908-v1',
            source_industry_version='industries-20260908-v1',
    ...
    def test_run_keeps_independent_execution_record(self):
        ...
        self.assertEqual(run.source_industry_version, 'industries-20260908-v1')
    ...
    def test_result_persists_non_fatal_analysis_warnings_for_api_reads(self):
        result = StockMoveResult.objects.create(
            business_date=date(2026, 9, 8),
            source_daily_price_version='daily-prices-20260908-v1',
            warnings=['股票 600001 未映射到开盘啦板块。'],
        )

        self.assertEqual(result.warnings, ['股票 600001 未映射到开盘啦板块。'])
