...
def _handle(handler, *, requested_date: bool = False):
    try:
        return handler()
    except CompleteMarketDataUnavailable as error:
        if requested_date:
            return ApiError(
                ErrorCode.DATA_NOT_AVAILABLE,
                '请求日期没有可用的大涨跌幅与大成交量个股数据。',
                http_status=404,
            ).as_response()
        return ApiError(
            ErrorCode.DATA_PREPARING,
            '大涨跌幅与大成交量个股数据正在准备中，请先运行管理命令。',
            http_status=202,
...
def _results(request):
    ...
    return _success(read_stock_moves(_optional_date(request)))
