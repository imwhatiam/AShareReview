...
import useEastmoneyData from './useEastmoneyData'
...
export default function EastmoneyPage({ apiClient }) {
  const [date, setDate] = useState('')
  const [days, setDays] = useState(1)
  const { phase, envelope } = useEastmoneyData({ apiClient, date, days })
  ...
  if (phase === 'error') {
    return <DataState state="error" message="东方财富数据暂时无法加载。" />
  }
  if (envelope?.error?.code === 'DATA_PREPARING') {
    return <DataState state="preparing" />
  }
  if (envelope?.error?.code === 'DATA_NOT_AVAILABLE') {
