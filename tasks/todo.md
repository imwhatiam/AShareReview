...
**规格：** `docs/specs/integration-spec.md`  
**计划说明：** `tasks/plan.md`
...
- [x] **修改文件范围：** `docs/data-sources/hithink-and-kaipanla-capability.md`（只读验证且不得记录凭据）。
- **具体结果：** 记录已批准的同花顺 REST 端点、`X-api-key` 鉴权、股票/交易日历/日线字段、最近一年逐股票前复权参数，以及开盘啦行业→股票链路和行业快照三字段约束；记录 API Key 实测最小权限与未知的 QPS/并发/日额度风险。
- **测试方式：** 对照公开文档逐项核验；在用户现有凭据环境中执行最小只读 REST 与开盘啦调用；敏感值只记录“可用/不可用”，不写入文档或日志。
- **验收标准：** ① 文档记录股票列表、交易日历、单股票最近一年 `interval=1d`/`adjust=forward` 日线及开盘啦行业→股票链路的成功证据；② 记录多行业归属；③ 未公开额度明确转化为低频、有限退避和旧版本保留的实现约束。
...
- **具体结果：** 锁定后端与前端依赖；登记五个数据库、缓存、安全、模块开关、同花顺、开盘啦和轻量修复预算配置；忽略所有 SQLite、缓存、日志、构建产物和虚拟环境。
...
- **具体结果：** 建立股票基础信息、交易日历和当前开盘啦行业—股票快照；统一标准股票代码。行业快照只含 `industry_code`、`industry_name` 和 `stock_codes` 三个业务字段，不保留历史有效期；同一股票允许出现在多个行业的股票列表中。
- **测试方式：** 模型约束测试覆盖唯一键、交易所枚举、三字段快照和无历史有效期字段；覆盖同一股票出现在多个行业记录。
- **验收标准：** ① 满足 AC-CORE-004/005/006；② 不存在跨数据库外键；③ 北交所可被正确标识而非深证。
- **潜在风险：** 同花顺代码格式和交易所字段语义尚未完全确认；开盘啦响应结构或行业分页规则变化。
- **依赖：** T03、T04、T01。
- **规模：** M（4 个文件）。
...
- **测试方式：** Django 测试客户端（`django.test.Client`，非 DRF）有效/无效登录、CSRF、Session 过期、普通用户、staff 用户、模块开关和健康检查无上游访问。
...
- **具体结果：** 按开盘啦行业→股票列表链路同步当前扁平行业快照；每行仅保存 `industry_code`、`industry_name` 和 JSON `stock_codes`，采用整批替换/发布语义，不保留历史有效期。请求的 `Date` 默认取最近一个交易日（历史端点对非交易日回 `errcode 1020 参数出错`，不能默认本地日期），`KAIPANLA_INDUSTRY_DATE` 可显式回补；发布版本的 `business_date` 即该交易日，命令成功输出会显示它。上游业务错误（如 `1020 参数出错`）会带 `errcode`/`errmsg` 抛出，不再只有笼统的 "request was rejected"。
- **测试方式：** 覆盖同一股票属于多个行业、缺成分、空成分、重复行业代码、分页、dry-run 和中途失败；确认链路响应不完整时命令失败。
- **验收标准：** ① 满足 AC-CORE-005/006；② 每个行业直接保存自己的股票列表；③ 映射版本可供派生模块引用。
- **潜在风险：** 开盘啦响应结构或分页语义变化；多行业归属导致下游聚合重复计数时实现错误。
- **依赖：** T09、T14、T15。
- **规模：** M（4 个文件）。
...
- [x] 股票、日历、开盘啦行业和映射命令 dry-run 测试通过。
- [x] 开盘啦行业→股票链路已验证可用；2026-09-09 真实验证确认资金流、行业列表及 2026-09-08 已发布股票快照在携带或省略全部 `KPL_*` 字段时均可用，适配器契约测试覆盖其分页与失败语义。
- [x] 2026-09-09 全量行业快照实测发现行业 `801058` 可合法返回空股票列表；同步契约改为持久化空 `stock_codes`，不阻断其他行业发布。
...
- **具体结果：** 使用批准的 POST URL、表单参数和 80 条分页；`KPL_DEVICE_ID`、`KPL_USER_ID`、`KPL_TOKEN` 均为可选环境值，只有非空值才作为请求字段发送；保留响应日期/时间语义，所有可变值来自环境。
...
- [x] **修改文件范围：** `backend/kaipanla/services/writer.py`、`backend/kaipanla/services/intraday.py`、`backend/kaipanla/services/read_path.py`、`backend/kaipanla/management/commands/fetch_kaipanla_sector_fund_flow.py`、`backend/kaipanla/tests/test_command.py`、`backend/kaipanla/tests/test_intraday.py`、`backend/kaipanla/tests/test_fallback.py`、`backend/kaipanla/tests/test_publication.py`。
- **具体结果：** 命令支持按公共交易日历判断的交易时段无参数模式、`--latest` 和 `--dry-run`；写入事务完成并通过分页完整性后才发布版本和失效本模块缓存。落库时刻统一由 `resolve_snapshot_slot()` 按运行时刻判定：盘中向前回退到所在 5 分钟槽（09:33 → 09:30、10:46 → 10:45、14:22 → 14:20），午休回退到 11:30，盘后（如 16:34）落在 15:00，非交易日与开盘前落在最近一个交易日的 15:00 并覆盖之；同一槽位重复写入为 upsert 覆盖。命令、`--latest` 与 Web 补齐共用该函数，非交易日不再写入当天 15:00 的假收盘快照。交易日判定分两层：周末（含调休上班的周末）一律休市，工作日的法定节假日用 `chinese-calendar` 判定，该库无对应年份数据时退回同花顺 `TradingDay` 日历（历史交易日与窗口计算仍以同花顺日历为准）。开盘啦资金流接口未获得经实际请求验证的历史快照参数支持，因此当前不提供 `--date`、`--snapshot-time` 或其他显式历史时间参数。
...
- [x] 无任何 `stock_moves`、`sector_momentum`、`hundred_day` 导入。
...
- **具体结果：** 提供 sectors、intraday、history、dates API；按缓存→数据库→一次有界远程修复读取；返回统一外壳、实际日期、版本、stale、partial/preparing 状态。远程修复仅限上海当日、单页、无重试、最多1,000行且超时不超过 `.env` 的硬上限；资金流命令不提供未被上游验证支持的 `--snapshot-time`。
...
- **具体结果：** 计算“涨幅>5%”和“全市场涨幅前5%”两套开盘啦行业前10；稳定截断；评分=`数量×平均涨幅×行业成交额占比`；记录未映射股票。
...
- **具体结果：** 聚合当前开盘啦行业新高/新低及股票明细；生成最多100个交易日结构化比例趋势；计算输入最多199个交易日位置；事务发布结果。
...
- **具体结果：** 统一上涨红、下跌绿、中性/错误色、排版和间距；提供加载、空、过期、准备中、部分成功、失败状态及旧数据/部分数据提示。
...
- **具体结果：** 迁移日期/1、5、10、20日控件、双排行勾选、分时和多日图表；每侧榜单展示前 10 条并置于图表下方；缺失点保持空值；共享组件通过属性接收模块数据，不直接调用具体 API。
- **测试方式：** 组件测试每侧前10、默认前5、勾选切换、时间轴、历史窗口和缺失点不画0；构建检查 ECharts。
- **验收标准：** ① 保留 `fundflow` 核心交互；② 共享层无 `kaipanla` API 导入；③ 同交易日保留选择所需接口可用。
- **潜在风险：** 过度抽象掩盖数据源 partial 差异；ECharts tooltip 使用未净化 HTML。
- **依赖：** T36。
- **规模：** M（5 个文件）。
...
- **具体结果：** 调用 `/api/kaipanla/` 系列接口，支持日期、当日/多日、排行选择、状态元数据；同一交易日保留选择，切换日期恢复默认前5；点任一统计窗口按钮（当日/5日/10日/20日）都会清空手选日期，窗口以最近一个已发布交易日为终点，日期控件显示实际生效的业务日期；用户显式选日期时该日期成为窗口终点；无数据时只隐藏图表与两张排行，日期与统计窗口控件保留。图表侧：折线图不渲染顶部图例，板块名与资金净额常驻每条折线右端；分时横轴刻度落在整十分钟上（09:30 起、下午 13:00 起、末刻度 15:00），不按索引抽稀；统计窗口分段按钮去掉外层背景边框并与日期控件上下对齐。
...
- [x] 开盘啦行业—股票快照通过独立配置 `KAIPANLA_INDUSTRY_API_URL` 请求历史端点 `apphis.longhuvip.com`；板块资金流继续使用 `KAIPANLA_API_URL` 的实时端点 `apphwshhq.longhuvip.com`。该修正源于对 `~/test.py` 已验证链路的复核。
...
### T39：实现大涨跌幅与大成交量个股页面（仅板块标签）

- [x] **修改文件范围：** `frontend/src/features/stock-moves/StockMovesPage.jsx`、`frontend/src/features/stock-moves/useStockMoves.js`、`frontend/src/features/stock-moves/StockGroupTable.jsx`、`frontend/src/features/stock-moves/StockMovesPage.test.jsx`。（`StockGroupTable.jsx` 后被 `StockMoveBoard.jsx` 取代并删除，现目录只有 `StockMoveBoard.jsx` / `StockMovesPage.jsx` / `useStockMoves.js` / `StockMovesPage.test.jsx`。）
- **具体结果：** 展示日期、分组统计与看板（当时为上证/深证四组；北交所在后续迭代补为独立第五组）、总数与全部去重代码；支持复制全部并对复制失败给出可见提示。
...
- **具体结果：** 自动验证四种 `core + 单业务 App` 配置的 check、迁移路由、命令发现、测试标签和 URL；扫描跨业务 import、跨库关系、禁用 URL、危险数据源和生产安全配置。
...
- **依赖：** T18、T23、T26、T29、T33、T42。
- **规模：** M（5 个文件）。

### T44：完成部署说明、验收追踪和受控运行时验收

- [x] **修改文件范围：** `README.md`、`docs/ops/deployment.md`、`docs/acceptance/traceability.md`、`tasks/todo.md`（仅更新完成状态）。
- **具体结果：** 记录本地/阿里云 `.env` 配置、五库迁移顺序、管理员创建、四组盘后 crontab 命令（后续可另装可选的盘中链路，见 `docs/ops/deployment.md` §6.1）、静态文件/HTTPS安全要求、手动备份责任和回滚方式；将全部规格 AC 映射到测试或人工证据。
...
- **验收标准：** ① 当期 98 条规格验收标准均有证据或明确待用户环境项（标准数量后续增至 109，以追踪表「标准数量」为准）；② 不声称已执行未获准的 Web 测试；③ 文档明确无自动备份、清理和外部通知。
