...
- [x] 开盘啦父行业→子行业→股票链路已验证可用；2026-09-09 真实验证确认资金流、父行业、子行业及 2026-09-08 已发布股票快照在携带或省略全部 `KPL_*` 字段时均可用，适配器契约测试覆盖其分页与失败语义。
- [x] 2026-09-09 全量行业快照实测发现子行业 `801058` 可合法返回空股票列表；同步契约改为持久化空 `stock_codes`，不阻断父行业及其他有效子行业发布。
